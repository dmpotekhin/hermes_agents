...
1. **Контроль длины сессии:**
   - При ~200 сообщениях в сессии предложи пользователю «начать новую сессию» (с кратким handoff), а не продолжать копить контекст.
   - Если сессия длится больше часа или контекст разросся — предложи `/compact` или `/compress here 50`.

2. **Playwright/browser snapshot:**
...
8. **Модели:** дефолтная модель — deepseek-v4-flash (дешевле в ~40 раз). deepseek-v4-pro — ТОЛЬКО для сложных архитектурных задач по явному запросу пользователя.
