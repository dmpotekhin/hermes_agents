#!/usr/bin/env python3
"""Credential pattern scanner — high-confidence pre-commit catch.

Источник: Agentic OS (https://github.com/KbWen/agentic-os, MIT License)
Файл: .agentcortex/tools/scan_credentials.py
Интегрирован в профиль Hermes developer.
...
hook is OPT-IN and bypassable with ``git commit --no-verify``. The regex CORRECTNESS
is CI-tested, but the ENFORCED control is CI TruffleHog (verified-secret detection).
...
Does NOT catch (by design — left to TruffleHog, or omitted to avoid false positives):
  ...
  * Ambiguous shapes — connection strings and JWTs.
...
# (name, compiled regex). DISTINCTIVE-PREFIX, high-confidence shapes only.
...
_SELF_SKIP = ("scan_credentials.py",)
...
def scan_text(text: str, label: str) -> list[tuple[str, int, str]]:
    ...
    A line containing ``pragma: allowlist secret`` is skipped — escape hatch for
    documented EXAMPLE tokens.
    """
...
def main() -> int:
    ...
    ap.add_argument("--range", metavar="A..B", dest="range_spec",
                    help="scan added lines of `git diff A..B`")
