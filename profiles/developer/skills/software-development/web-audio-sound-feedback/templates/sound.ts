...
// Для useSyncExternalStore(subscribe, isMuted)
...
// Разблокировать аудио на первом жесте (pointerdown/keydown).
export function unlock(): void {
  const c = ensureCtx();
  if (c && c.state === 'suspended') c.resume().catch(() => {});
}

// Примитивы синтеза. Огибающие от 0.0001 (exponentialRamp не принимает 0).
...
const SYNTH: Record<SoundName, () => void> = {
  ...
  reveal() {
    ...
    if (!c) return;
    const t = c.currentTime;
    tone(659.25, t, 0.25, { gain: 0.22 });
    tone(880, t + 0.08, 0.5, { gain: 0.2 });
  },
  tick() {
    const c = ensureCtx();
    if (!c) return;
