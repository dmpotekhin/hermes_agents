...
- **Заглушки доказывают куски — живой прогон доказывает проводку.** После зелёных тестов с фейками обязателен ОДИН прогон на реальных провайдерах с дешёвым синтетическим входом (генерировать локально, а не искать/покупать внешние данные): он ловит то, чего не видят unit-тесты — bootstrap окружения (`load_dotenv` живёт в точке входа приложения, внешний скрипт обязан загрузить `.env` сам), фактическую форму ответа провайдера, реальные пути артефактов, число и стоимость вызовов. Заглушки + exit 0 не доказывают, что подсистема вообще запускается из точки входа.
- **Инструмент, чей успех выражается пустым выводом, проверять положительным контролем.** `exit 0` без вывода — это и «чисто», и «не запустился» (сканер секретов, grep-страж, валидатор). Один раз подать заведомо «грязный» вход (тестовый файл с известными фейковыми ключами/паттерном), убедиться, что находка есть и код возврата ненулевой, затем убрать контрольный файл и из индекса, и с диска — и только после этого верить чистому прогону на реальном входе.
- **Verification commands can produce no usable output.** When a probe/gate command returns empty (or the run is blocked before it can report), that is NOT a pass: say plainly which claim stays unverified and offer the exact path `(<run dir>`, artifact list) the user can inspect — never let "exit 0" stand in for evidence you did not see.
- **Правки файлов — мелкими порциями, и сразу гейт.** Крупный `new_string`/`content` может лечь в файл ОБРЕЗАННЫМ: инструмент отчитывается об успехе, а в теле остаётся буквальный маркер (вида `...truncated]`), синтаксис файла сломан и вы об этом не знаете до следующего чтения. Дробить правку на несколько патчей с короткими уникальными якорями; после каждой пачки прогонять гейт репозитория (`ruff check`, `python -m compileall`, целевой pytest) и `grep -rn "truncated\]"` по затронутым файлам. Не «отрефакторить весь пакет одним патчем».
...
- **Before reporting a security finding, walk the data flow to its barrier and cite it.** For each "untrusted input → sensitive operation" chain (file write, ZIP entry name, directory path, exec, network), follow it to the concrete guard in the repo — an existing sanitizer (`sanitize_filename`, `safe_filename`, slugify), a safe-stem helper for run/cache dirs, an existing untrusted-content wrapper (`wrap_external`, `SYSTEM_GUARD`, `<<<EXTERNAL_CONTENT>>>`), JSON parsed as data (`json`, a tolerant `_extract_json`) and never executed, or keys/HTTP confined to the existing router. An existing barrier on the path means there is NO finding — say so and cite function + line. A barrier present but skipped on one path (e.g. `cleanup_run(request_id)` while `run_dir_for` sanitizes) is a hardening note, not a finding. Also check WHO controls each input: internally generated values (`request_id`, run-directory names, internal ids) are not user input, so traversal through them is not exploitable — state that plainly instead of filing it as a low finding.
