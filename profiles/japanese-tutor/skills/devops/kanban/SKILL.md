---
name: kanban
description: Complete Kanban workflow — orchestrator decomposition playbook, worker pitfalls and edge cases, lifecycle automation. Covers orchestrator routing and worker execution for Kanban-driven multi-agent systems.
version: 3.0.0
platforms: [linux, macos, windows]
environments: [kanban]
metadata:
  hermes:
    tags: [kanban, multi-agent, orchestration, routing, worker, collaboration, workflow]
    related_skills: []
---
...
## Section: Worker Execution Guide

This section covers the worker perspective — what to do when you're spawned as a Kanban worker.

### Workspace Handling

Your workspace kind determines how to behave inside `$HERMES_KANBAN_WORKSPACE`:
...
| `worktree` | Git worktree | Commit work here; if `.git` missing, run `git worktree add` |

### Good Summary + Metadata Shapes

**Coding task:**
```python
kanban_complete(summary="shipped rate limiter — token bucket, 14 tests pass", metadata={
...
**Research task:**
```python
kanban_complete(summary="3 libraries reviewed; vLLM wins on throughput", metadata={
    "recommendation": "vLLM", "sources_read": 12,
})
```

**Review task:**
```python
kanban_complete(summary="PR #123 reviewed; 2 blocking issues", metadata={
    "pr_number": 123, "approved": False,
    "findings": [{"severity": "critical", "file": "api/search.py", "line": 42, "issue": "SQL injection"}],
})
```

### Claiming Cards You Created

Pass `created_cards` on `kanban_complete` — the kernel verifies each id was actually created by your profile. Never invent ids from prose.

```python
c1 = kanban_create(title="remediate SQL injection", assignee="security-worker")
c2 = kanban_create(title="fix CSRF middleware", assignee="web-worker")
kanban_complete(summary="Review done; spawned fixes", created_cards=[c1["task_id"], c2["task_id"]])
```

### Block Reasons That Get Answered Fast

Bad: `"stuck"` — no context.
Good: one sentence naming the specific decision needed. Leave long context as a comment.

```python
kanban_comment(body="Full context: user IPs from Cloudflare headers, some behind NAT...")
kanban_block(reason="Rate limit key: IP (simple, NAT-unsafe) or user_id (requires auth)?")
```

### Heartbeats

Name concrete progress: `"epoch 12/50, loss 0.31"`, `"scanned 1.2M/2.4M rows"`. Not `"still working"`. Skip for tasks under ~2 minutes.

### Retry Scenarios

Check `kanban_show` on startup — prior runs' `outcome` tells you what didn't work:
- `timed_out` — previous hit max runtime, chunk the work
- `crashed` — OOM/segfault, reduce memory
- `spawn_failed` — config issue, block for human
- `reclaimed` — operator archived the task; check status before proceeding
...
- Don't rely on `hermes kanban <verb>` CLI in containerized backends — use the tools
- Tenant isolation: prefix memory entries with `$HERMES_TENANT`

### CLI Fallback

Every tool has a CLI equivalent for scripting:
