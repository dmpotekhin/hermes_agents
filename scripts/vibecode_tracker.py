...
# Auto-detect profile: if HERMES_HOME is a profile dir, extract name
_hermes_home = os.environ.get("HERMES_HOME", "")
if _hermes_home and "profiles" in Path(_hermes_home).parts:
    PROFILE = Path(_hermes_home).name
else:
...
OBSIDIAN_VAULT = Path(os.path.expanduser(
    os.environ.get("VIBECODE_OBSIDIAN_VAULT",
        "~/Documents/Obsidian/Brain/notes/vibecoding"
        if os.name == "nt" else
...
def cmd_start(project="unknown"):
    ...
    if state["current"]:
        # Already running — close it properly so its segments aren't lost.
        prev = state["current"]
        last = parse_iso(prev["last_segment"])
        gap = (NOW - last).total_seconds()
        if 30 < gap <= 600:
            prev["segments"].append({
                "from": prev["last_segment"],
                "to": now_iso,
        ...
        prev["ended"] = now_iso
        state.setdefault("sessions", []).append(prev)
...
def cmd_stop():
    ...
    # Log final segment, respecting auto-pause (>10 min idle = pause, not work)
    last = parse_iso(state["current"]["last_segment"])
    ...
    # Summary managed manually (backfill merge), not auto-overwritten
...
def cmd_stats(period="today"):
    ...
    # Include the live session so CLI stats match the daily log
    sessions = list(all_sessions)
    if state.get("current"):
        sessions.append(state["current"])
...
def _write_daily_log(date_str, state=None, completed_session=None):
    ...
    # Read existing file to preserve backfill section
    existing = ""
    if path.exists():
        existing = path.read_text(encoding="utf-8")

    # Split at LIVE marker if present
    backfill_part = existing
    if "<!-- LIVE -->" in existing:
    ...
    live_lines.append(f"### {profile} (live)\n")
    live_lines.append(f"| # | Начало | Длит. | Проект |")
    live_lines.append(f"|---|--------|-------|--------|")
    ...
    for i, (sess, seg, _) in enumerate(all_segments, 1):
        ...
        proj = seg["project"]
        live_lines.append(f"| {i} | {start} | {dur} | {proj} |")

    live_lines.append("")
    live_lines.append(f"**Сегодня (live): {fmt_duration(total)}** | **Сегментов: {total_commits}**")
    live_lines.append("")

    path.write_text(backfill_part + "\n\n" + "\n".join(live_lines) + "\n", encoding="utf-8")
